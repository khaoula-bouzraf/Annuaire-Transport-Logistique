                ...
                <?php
                  ...
                  if($expired_date >= strtotime(date('dMY'))){ ?>
                    <?php if($booking['status'] == 0){ ?>
                      <a href="<?php echo site_url('user/booking_request_hotel/approved/'.$booking['id']); ?>" class="btn btn-default btn-sm btn-icon icon-left">
                        <i class="entypo-check"></i>
                        <?php echo get_phrase('approve'); ?>
                      </a>
                    <?php }else{ ?>
                      <a href="<?php echo site_url('user/booking_request_hotel/pending/'.$booking['id']); ?>" class="btn btn-default btn-sm btn-icon icon-left">
                        <i class="entypo-check"></i>
                        <?php echo get_phrase('pending'); ?>
                      </a>
                    <?php } ?>
                  <?php } ?>
                  <a href="#" class="btn btn-danger btn-sm btn-icon icon-left" onclick="confirm_modal('<?php echo site_url('user/booking_request_hotel/delete/'.$booking['id']); ?>');">
