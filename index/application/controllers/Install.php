...
class Install extends CI_Controller {
  ...
  function step3($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'configure_database') {
      $hostname = sanitizer($this->input->post('hostname'));
      $username = sanitizer($this->input->post('username'));
      $password = sanitizer($this->input->post('password'));
      $dbname   = sanitizer($this->input->post('dbname'));
  ...
  function finalizing_setup($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'setup_admin') {
      $admin_data['name']         = sanitizer($this->input->post('name'));
      $admin_data['email']        = sanitizer($this->input->post('email'));
      $admin_data['password']     = sha1(sanitizer($this->input->post('password')));
      ...
      $admin_data['social']    = json_encode($social_links);
      $admin_data['wishlists'] = '[]';
  ...
  function success($param1 = '') {
    ...
    $this->load->database();
    $admin_email = $this->db->get_where('user', array('id' => 1))->row()->email;
