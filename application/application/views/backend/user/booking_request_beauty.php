    ...
    <div class="panel panel-primary" data-collapsed="0">
      <div class="panel-heading">
        <div class="panel-title">
          <?php echo get_phrase('booking_request_for_beauty'); ?>
        </div>
      </div>
      <div class="panel-body">
        <table class="table table-bordered datatable">
          ...
          <tbody>
            ...
            <?php $bookings = $this->db->get_where('booking', array('user_id' => $this->session->userdata('user_id'), 'listing_type' => 'beauty'))->result_array();
            $count = 1;
            ...
            <tr>
              ...
              <td>
                ...
                foreach($informations as $key => $value){
                  if($key == 'service'){
                    ?>
                    <span><?php echo get_phrase($key); ?> : <?php echo $this->db->get_where('beauty_service', array('id' => $value))->row('name'); ?></span><br>
                    <?php
                  };
              ...
              <td class="text-center">
                ...
                if($expired_date >= strtotime(date('dMY'))){ ?>
                  <?php if($booking['status'] == 0){ ?>
                    <a href="<?php echo site_url('user/booking_request_beauty/approved/'.$booking['id']); ?>" class="btn btn-default btn-sm btn-icon icon-left">
                      <i class="entypo-check"></i>
                      <?php echo get_phrase('approve'); ?>
                    </a>
                  <?php }else{ ?>
                    <a href="<?php echo site_url('user/booking_request_beauty/pending/'.$booking['id']); ?>" class="btn btn-default btn-sm btn-icon icon-left">
                      <i class="entypo-check"></i>
                      <?php echo get_phrase('pending'); ?>
                    </a>
                  <?php } ?>
                <?php } ?>
                <a href="#" class="btn btn-danger btn-sm btn-icon icon-left" onclick="confirm_modal('<?php echo site_url('user/booking_request_beauty/delete/'.$booking['id']); ?>');">
